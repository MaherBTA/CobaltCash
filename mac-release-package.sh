...
PACKAGE=XCC-client-${VERSION}
echo PACKAGE="${PACKAGE}"
CHANGELOG=XCC-client-${VERSION}.changelog.txt
OBFUSCATE=$2
MACVERSION=$3
if [ -x ${MACVERSION} ];
then
...
FILES="${FILES} CobaltCash.exe CobaltCashservice.exe"
...
echo copy resources
cp installer/lib/JavaExe.exe CobaltCash.exe
cp installer/lib/JavaExe.exe CobaltCashservice.exe
cp -a ${FILES} nxt
